    ...
    Webitel = function(parameters) {
        ...
        var WebitelEventMixin = function() {
            ...
            var on = function(eventName, handler, cb) {
                ...
                commandEvent(eventName, WebitelCommandTypes.Event.Subscribe, function(response) {
            ...
            var off = function(eventName, cb) {
                ...
                commandEvent(eventName, WebitelCommandTypes.Event.UnSubscribe, function(response) {
                    if (response.status === WebitelCommandResponseTypes.Success)
...
//            AccountRole: 'ACCOUNT_ROLE',
        ...
        var WebitelCall = function(key, channel, state) {
            ...
            this.getCalleeNumber = function () {
                ...
                var callerCallee = chn[WebitelCallChanelVariables.CallerCalleeNumber],
                    callerCaller = chn[WebitelCallChanelVariables.CallerCallerNumber];
                if (callerCallee) {
                    callerCallee = callerCallee.replace('@')[0];
                };
                if (callerCaller) {
                    callerCaller = callerCaller.replace('@')[0];
                };
                if (chn[WebitelCallChanelVariables.WJsOriginate] &&
                    callerCallee === callerCaller) {
            ...
            this.getDisplayNumber = function () {
                return (that.getDirection() == WebitelCallDirectionTypes.Outbound)
        ...
        function handleChannelHangupCompleteEvent (id, e) {
            ...
            if (webCall) {
                ...
                try {
                    ...
                    if (WebrtcCalls.collection.length > 0) {
                        cur_call = WebrtcCalls.collection[0]['session']
                    };
                } catch (e) {}
            };
                ...
                onDialogState: function(d) {
                    ...
                    switch (d.state) {
                        ...
                        case WebitelVerto.enum.state.ringing:
                            ...
                            cur_call.params.tag = webitel.video.addIncVideo(d.callID);
                            break;
...
//                    that._status = ConnectionStatus.Disconnected;
        ...
        var resultInterface = {
             ...
             *  @cause {WebitelUserAwayCauseTypes}: статус.
             *  @tag {String}: дополнительная информация.
             **/
