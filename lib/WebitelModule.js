...
//        OnHook: '"ONHOOK"'
    ...
    Webitel = function(parameters) {
        ...
        var WebitelEventMixin = function() {
            ...
            var on = function(eventName, handler, cb) {
...
//            AccountRole: 'ACCOUNT_ROLE',
        ...
        var WebitelCall = function(key, channel, state) {
            ...
            this.getCallerNumber = function () {
                ...
                if (_actual['variable_webitel_att_xfer']) {
                    return _actual['Caller-Caller-ID-Name']
                }
            ...
            this.getVisibleHold = function () {
                return that.getActualChannel()[WebitelCallChanelVariables.WAttXfer];
            };

            this.getXTransferChannelId = function () {
                return that.getActualChannel()[WebitelCallChanelVariables.UUID] + '_att_xfer';
                //return that.getActualChannel()[WebitelCallChanelVariables.WJsXTransfer];
            };
            // TODO
            this.setAttXferCalleNumber = function (number) {
                that['AttXferCalleNumber'] = number;
            };

            this.getAttXferCalleNumber = function () {
                return that['AttXferCalleNumber'];
            };
            // END TODO
            ...
            this.setAdditionInfo = function (result) {
                ...
                result['att_xfer_call'] = channel[WebitelCallChanelVariables.WAttXfer];
            };
            ...
            switch (attrs['Event-Name']) {
                ...
                case WebitelUserEventTypes.AccountStatus:
                    ...
                    if (user) {
                        ...
                        user.setStatus(attrs['Account-Status'], attrs['Account-Status-Descript'] || '');
                    }
                    break;
            ...
            try {
                switch (attrs['Event-Name']) {
                    ...
                    case WebitelCallEventTypes.RecordStop:
                        ...
                        break;
                    case WebitelCallEventTypes.ChannelExecute:
                        handleChannelExecuteEvent(channelId, attrs);
                        break;
                    case WebitelCallEventTypes.ChannelExecuteComlete:
                        handleChannelExecuteComleteEvent(channelId, attrs);
                        break;
        ...
        function handleChannelExecuteEvent(id, e) {
            if (e['Application'] == "att_xfer") {
                //
                var callee = '';
                try {
                    callee = /user\/(\d+)[@|$]/g.exec(decodeURIComponent(e['Application-Data']))[1]
                } catch (e) {
                    //TODO
                }
                //
                var virtualCallId = id + '_att_xfer';
                var virtualChannel = e;
                virtualChannel[WebitelCallChanelVariables.CallerCalleeNumber] = callee;
                virtualChannel[WebitelCallChanelVariables.CallerCallerNumber] = account.split('@')[0];
                virtualChannel[WebitelCallChanelVariables.UUID] = virtualCallId;
                ChanelCalls.add(virtualCallId, virtualChannel);
                var virtualCall = new WebitelCall(virtualCallId, virtualChannel, WebitelCallStates.Rinding);
                OngoingCalls.add(virtualCall);
                virtualCall.setState(WebitelCallStates.Answered);
            };
        };

        function handleChannelExecuteComleteEvent(id, e) {
            if (e['Application'] == "att_xfer") {
                var virtualCallId = id + '_att_xfer';
                try {
                    var call = OngoingCalls.get(virtualCallId);
                    OnWebitelCallHangup.trigger(call.getJSONObject());
                } catch (e) {
                    // TODO
                } finally {
                    ChanelCalls.remove(virtualCallId);
                    OngoingCalls.remove(virtualCallId);
                }
            };
        };
        ...
        function handleChannelCallState(id, e) {
            ...
            var callId = getCallIdFromEvent(channel);
            updateChannelParameters(channel, e);
//            var call = OngoingCalls.get(callId);
//            switch (e[WebitelCallChanelVariables.CallStateName]) {
//                case "RINGING":
//                case "EARLY":
//                    if (!call) {
//                        var newCall = new WebitelCall(callId, e, WebitelCallStates.Rinding);
...
//            var xferCall = OngoingCalls.get(call.getXTransferChannelId());
//            if (xferCall && e[WebitelCallChanelVariables.CallerCalleeNumber] == xferCall.getCalleeNumber()) {
//                xferCall.setState(WebitelCallStates.Answered);
//            };
        };
                    ...
                    switch (d.state) {
                        ...
                        case WebitelVerto.enum.state.active:
                            var webCall = WebrtcCalls.get(d.callID),
                                _channel = ChanelCalls.get(d.callID);
...
//                    that._status = ConnectionStatus.Disconnected;
            ...
            makeCall: function(extension, useVideo) {
                ...
                if (webrtc && webrtcPhone) {
                    ...
                    /* var options = {
                     ...
                     webrtcPhone.call('sip:' + extension + '@' + webrtc_domain, options);*/
                } else {
                    var params = {
                        "extension": extension,
            ...
            answer: function(callUUID, useVideo) {
                ...
                /* V2
                 ...
                 var call = OngoingCalls.get(callUUID);
                 if (!call) return false;
                 var cmd = new WebitelCommand(WebitelCommandTypes.Answer, {
                 'call-uuid': callUUID,
                 'user-id': account
                 },
                 function(sender) {
            ...
            sendMessage: function (extension, text, succeededCb, failedCb) {
                ...
                if (typeof  succeededCb == "function") eventHandlers['succeeded'] = succeededCb;
                if (typeof  failedCb == "function") eventHandlers['failed'] = failedCb;
            ...
            setStatusReady: function() {
                ...
                /*
                 var readyStatusCommand = new WebitelCommand(WebitelCommandTypes.CahangeStatus, {
                 set: WebitelAccountStatusTypes.Ready
                 },
                 function() {
                 console.log('Status successfully changed to Ready!');
                 }
                 );
                 readyStatusCommand.execute(); */
            },

            setStatusBusy: function(cause, tag) {
                ...
                /*
                 params = {
                 set: WebitelAccountStatusTypes.Busy,
                 cause: cause
                 };
                 if (tag) {
                 params['tag'] = tag;
                 }
                 var busyStatusCommand = new WebitelCommand(
                 WebitelCommandTypes.CahangeStatus,
                 params,
                 function() {
                 console.log('Successfuly changed status to Busy!');
                 }
                 );
                 busyStatusCommand.execute();
                 */
            },
            ...
            attendedTransferCall: function(callUUID, destination) {
                ...
                /*if (webrtc && webrtcPhone) {
                 ...
                 'extraHeaders': [ 'X-WebitelXTransfer: ' + callUUID]
                 };
                 webrtcPhone.call('sip:' + destination + '@' + webrtc_domain, options);
                 return;
                 };*/
                call.setAttXferCalleNumber(destination);
                var channel = call.getActualChannel();
                var channelUUID = channel['Unique-ID'];
                var cmd = new WebitelCommand('att_xfer', {
                    'channel-uuid': channelUUID,
                    'destination': destination + '@' + account.split('@')[1],
                    'user': account,
                    //'auto_answer_param': ['media_webrtc=true'],
                ...
                /*
                 var channel = call.getActualChannel();
                 var channelUUID = channel['Unique-ID'];
                 var cmd = new WebitelCommand('att_xfer2', {
                 'channel-uuid': channelUUID,
            ...
            cancelTransfer: function (callIdA, callIdB) {
                ...
                /*
                 ...
                 var webCalls = WebrtcCalls.collection,
                 kill = true;
                 for (var key in webCalls) {
                 if (webCalls[key]['call-uuid'] == callIdB) {
                 webCalls[key]['session'].hangup();
                 kill = false;
                 break;
                 };
                 };
            ...
            getVariable: function(channelUUID, variable, inleg, callback) {
                if (!channelUUID || !variable) return false;
        ...
        WebitelConnection.onReceivedMessage.subscribe(function(message) {
            ...
            if (jsonResponse['webitel-event-name']) {
                ...
                try {
                    if (commandResponse.status == WebitelCommandResponseTypes.Success || currentCommand.commandName == "dtmf") {
        ...
        var resultInterface = {
             ...
             **/
            onAgentStatusChange: OnWebitelUserStatusChange.subscribe,
            unAgentStatusChange: OnWebitelUserStatusChange.unsubscribe,
